...
Return ONLY the JSON object, no other text.`
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      activeCount: active.length, wonCount: won.length, totalCount: opps.length,
    };
  }, [opps]);
    ...
    <div style={{ minHeight: "100vh", background: "#080B12", color: "#D4DAE8", fontFamily: "'IBM Plex Mono', 'Courier New', monospace" }}>
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        .btn-email{background:#A78BFA22;border:1px solid #A78BFA44;color:#A78BFA;cursor:pointer;padding:9px 18px;border-radius:6px;font-family:inherit;font-size:11px;font-weight:500;letter-spacing:.08em;transition:all .2s}
        .btn-email:hover{background:#A78BFA33}
        ...
        .opp-card{background:#0E1220;border:1px solid #1A2035;border-radius:8px;padding:14px 16px;transition:all .15s;cursor:pointer}
        ...
        .modal-wide{background:#0E1220;border:1px solid #2A3550;border-radius:12px;padding:28px;width:700px;max-height:90vh;overflow-y:auto}
        ...
        .file-row{display:flex;align-items:center;justify-content:space-between;padding:10px 12px;background:#0A0D16;border:1px solid #1A2035;border-radius:6px;gap:10px}
        .side-panel{position:fixed;right:0;top:54px;bottom:0;width:380px;background:#0C0F1A;border-left:1px solid #1A2035;padding:24px;overflow-y:auto;z-index:40}
      ...
      <div style={{ borderBottom: "1px solid #1A2035", padding: "0 32px", display: "flex", alignItems: "center", justifyContent: "space-between", height: 54, position: "sticky", top: 0, background: "#080B12", zIndex: 50 }}>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button className="btn-email" onClick={() => setShowEmailParser(true)}>⚡ Parse Email</button>
          <button className="btn-primary" onClick={openAdd}>+ New Deal</button>
        </div>
      </div>

      <div style={{ padding: "28px 32px", maxWidth: selectedDeal ? "calc(100% - 400px)" : 1200, margin: "0 auto", transition: "max-width .2s" }}>
                  ...
                  <div className="card">
                    ...
                    <div style={{ display: "flex", flexDirection: "column" }}>
                      {[...opps].sort((a, b) => (b.date || "").localeCompare(a.date || "")).slice(0, 6).map((o, i) => {
                        ...
                        return (
                          <div key={o.id} style={{ cursor: "pointer" }} onClick={() => openDeal(o)}>
                    ...
                    return (
                      <div key={stage} style={{ minWidth: 158 }}>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                          {list.map(o => (
                            <div key={o.id} style={{ background: "#0E1220", border: `1px solid ${c.color}25`, borderRadius: 7, padding: 12, cursor: "pointer" }} onClick={() => openDeal(o)}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 20 }} className="fade-up">
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {filtered.map(o => {
                    const c = STAGE_CONFIG[o.stage];
                    return (
                      <div key={o.id} className="opp-card" style={{ display: "grid", gridTemplateColumns: "2fr 2fr 1fr 1fr 1fr auto", gap: 12, alignItems: "center" }} onClick={() => openDeal(o)}>
                        <div>
      ...
      {/* SIDE PANEL — Deal Detail + Files */}
      {selectedDeal && (
        <div className="side-panel slide-in">
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
            <div style={{ fontFamily: "'Instrument Serif', serif", fontSize: 16, color: "#E8EDF8", fontStyle: "italic" }}>Deal Detail</div>
            <button className="btn-ghost" onClick={() => setSelectedDeal(null)}>✕</button>
          </div>

          {/* Deal info */}
          <div style={{ marginBottom: 20 }}>
            <div style={{ fontSize: 14, color: "#C0CAE0", fontWeight: 500, marginBottom: 6 }}>{selectedDeal.name}</div>
            <div style={{ fontSize: 11, color: "#3A4560", marginBottom: 10 }}>{selectedDeal.contact}</div>
            <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 10 }}>
              <span className="pill" style={{ background: STAGE_CONFIG[selectedDeal.stage].bg, color: STAGE_CONFIG[selectedDeal.stage].color }}>{selectedDeal.stage}</span>
              <span style={{ fontFamily: "'Instrument Serif', serif", fontSize: 18, color: "#E8EDF8" }}>{fmt(selectedDeal.value)}</span>
            </div>
            {selectedDeal.notes && <div style={{ fontSize: 11, color: "#6B7A99", lineHeight: 1.6, background: "#0A0D16", padding: "10px 12px", borderRadius: 6, border: "1px solid #1A2035" }}>{selectedDeal.notes}</div>}
          </div>

          <div style={{ display: "flex", gap: 8, marginBottom: 20 }}>
            <button className="btn-ghost" style={{ flex: 1 }} onClick={() => openEdit(selectedDeal)}>✎ Edit Deal</button>
            <button className="btn-ghost" style={{ color: "#F87171", borderColor: "#F8717120" }} onClick={() => deleteOpp(selectedDeal.id)}>✕</button>
          </div>

          <hr style={{ border: "none", borderTop: "1px solid #1A2035", marginBottom: 20 }} />

          {/* Files */}
          <div style={{ marginBottom: 14, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div style={{ fontSize: 10, letterSpacing: ".1em", textTransform: "uppercase", color: "#3A4560" }}>Files & Attachments</div>
            <label style={{ cursor: "pointer" }}>
              <input type="file" style={{ display: "none" }} onChange={uploadFile} />
              <span className="btn-ghost" style={{ color: "#60A5FA", borderColor: "#60A5FA33", fontSize: 11, padding: "4px 10px" }}>
            ...
            {dealFiles.map(file => (
              <div key={file.id} className="file-row">
                <div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 0 }}>
                  ...
                  <a href={file.url} target="_blank" rel="noopener noreferrer" style={{ fontSize: 12, color: "#60A5FA", textDecoration: "none", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{file.name}</a>
                </div>
                <button className="btn-ghost" style={{ color: "#F87171", borderColor: "#F8717120", flexShrink: 0 }} onClick={() => deleteFile(file.id, file.url)}>✕</button>
              </div>
            ))}
            {dealFiles.length === 0 && (
              <div style={{ fontSize: 11, color: "#2A3550", textAlign: "center", padding: "20px 0" }}>No files yet — upload a proposal, contract, or any relevant doc</div>
            )}
          </div>
        </div>
      )}
      ...
      {/* EMAIL PARSER MODAL */}
      {showEmailParser && (
        <div className="modal-bg" onClick={e => e.target === e.currentTarget && setShowEmailParser(false)}>
          <div className="modal-wide fade-up">
            <div style={{ fontFamily: "'Instrument Serif', serif", fontSize: 20, color: "#E8EDF8", marginBottom: 6, fontStyle: "italic" }}>Parse Email</div>
            <div style={{ fontSize: 11, color: "#3A4560", marginBottom: 20 }}>Paste any email below — AI will extract the deal details automatically</div>
            <textarea
              className="fi"
              rows={12}
              value={emailText}
              onChange={e => setEmailText(e.target.value)}
              placeholder="Paste the full email here — subject, body, sender info…"
              style={{ resize: "vertical", lineHeight: 1.6 }}
            />
            <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 16 }}>
              <button className="btn-ghost" style={{ padding: "8px 16px" }} onClick={() => { setShowEmailParser(false); setEmailText(""); }}>Cancel</button>
              <button className="btn-primary" disabled={parsing || !emailText.trim()} onClick={parseEmail}>
                {parsing ? "Parsing…" : "⚡ Extract Deal Info"}
              </button>
            </div>
          </div>
        </div>
